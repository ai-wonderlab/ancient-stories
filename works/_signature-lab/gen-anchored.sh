...
# ANCHOR override per-zone: export ANCHOR=/path/to/zone-anchor.png
# Bootstrap a new colour family (no anchor yet): export ANCHOR=none  -> text-only /generations
ANCHOR="${ANCHOR:-$LAB/anchors/MASTER-ANCHOR.png}"
SIZE="${SIZE:-1024x1536}"
...
if [ "$ANCHOR" = "none" ]; then
  # BOOTSTRAP a new colour-family anchor: text-only, self-contained signature lock.
  BOOT="One strict signature illustration identity for a mythology animation channel — drawn by one hand. Flat colour masses in only 2-3 tones; ONE uniform charcoal contour line; 100% flat fills; single global paper-grain; generous empty cream negative space. DISSOLVE DEVICE (the brand): the dominant mass keeps a clean solid edge, then whole SOLID dots break off and float into empty cream — dots ALWAYS sparse and well-separated, never overlapping/clustered, NEVER cream holes punched inside the mass. Composition: dominant mass on one vertical edge ~40% of frame, calm negative space opposite, low horizon, one tiny solid-charcoal witness figure for scale. KILL: no crosshatching, no internal hatching, no engraved/relief texture, no hairlines, no text, no letters, no crowd, no photoreal. SUBJECT: ${SUBJ}"
  jq -n --arg p "$BOOT" --arg s "$SIZE" '{model:"gpt-image-2",prompt:$p,size:$s,n:1}' > /tmp/anch_${ROUND}_${LABEL}.json
  http=$(curl -sS --max-time 300 --retry 1 --retry-all-errors -w "%{http_code}" -o /tmp/respb_${ROUND}_${LABEL}.json \
    https://api.openai.com/v1/images/generations \
    -H "Authorization: Bearer $OPENAI_API_KEY" -H "Content-Type: application/json" \
    --data @/tmp/anch_${ROUND}_${LABEL}.json 2>/dev/null)
  RESP=/tmp/respb_${ROUND}_${LABEL}.json
else
  ...
  RESP=/tmp/anch_${ROUND}_${LABEL}.json
fi

if [ "$http" = "200" ]; then
  jq -r '.data[0].b64_json' "$RESP" | base64 -d > "$FILE"
  echo "OK   sig-${ROUND}-${LABEL}  (anchor=$(basename "$ANCHOR"))"
else
  echo "FAIL ${LABEL} http=$http $(head -c 220 "$RESP" | tr -d '\n')"
fi
